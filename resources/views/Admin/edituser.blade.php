    ...
    <div class="body-wrapper">
      ...
      <div class="container mt-5">
        <div class="row justify-content-center">
          <div class="col-md-5">
            <form action="{{route('users.comp',$User->id)}}" method="POST">
              @csrf
            <div class="mb-3">
              ...
              <div class="mb-3">
                @if ($role->isNotEmpty())
                    @foreach ($role as $permission)
                        <label for="{{ $permission->id }}"><input {{$info->contains($permission->id) ? 'checked' : ''}} type="checkbox" id="{{ $permission->id }}" name="role[]" value="{{ $permission->name }}" /> {{ $permission->name }}</label>
                    @endforeach
                @endif
            </div>
